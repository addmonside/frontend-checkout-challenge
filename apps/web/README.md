...
- Каталог, серверная корзина, оформление с расчётом доставки.
- Создание заказа: после оформления заказ с картой ведёт на отдельную страницу
  оплаты `/orders/[orderId]/payment`, заказ с наличными — сразу на страницу заказа.
- Оплата картой по шагам API: создание попытки (`POST /orders/{id}/payments`,
  новый `Idempotency-Key`), выбор тестовой карты из `GET /sandbox`, запуск
  имитации (`POST /payments/{id}/simulations`) и опрос статуса с паузой из
  `Retry-After` до `succeeded`/`failed`/`cancelled`.
- Успех подтверждается заказом: перед переходом на страницу успеха заказ
  перечитывается, нужны `status=paid` и `paymentStatus=succeeded`.
- Отказ и отмена различимы, после каждого можно создать новую попытку с новым
  ключом. Незавершённая оплата восстанавливается после перезагрузки.
- Все запросы идут через общий слой `shared/api` (единая отправка, разбор тела,
  ошибок и заголовков), операции оплаты — через `services/payments`.
